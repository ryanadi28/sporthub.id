<x-guest-layout>
    <x-authentication-card>
        ...
        <div class="text-center mb-6">
            ...
            <p class="text-sm text-slate-500 mt-1">Booking lapangan jadi lebih mudah</p>
        </div>
        ...
        @if (session('status'))
            <div class="mb-4 bg-teal-50 border border-teal-200 rounded-lg p-3 flex items-center space-x-2">
                ...
                <span class="text-sm text-teal-800 font-medium">{{ session('status') }}</span>
            </div>
        @endif

        {{-- Error Message --}}
        @if (session('errormsg'))
            <div class="mb-4 bg-red-50 border border-red-200 rounded-lg p-3 flex items-center space-x-2">
                <svg class="w-5 h-5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
                <span class="text-sm text-red-800 font-medium">{{ session('errormsg') }}</span>
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            <div class="flex items-center justify-between">
                <label for="remember_me" class="flex items-center cursor-pointer">
                    <x-checkbox id="remember_me" name="remember" class="rounded border-slate-300 text-teal-600 focus:ring-teal-500" />
                    <span class="ml-2 text-sm text-slate-600 font-medium">Ingat saya</span>
                </label>

                @if (Route::has('password.request'))
                    <a class="text-sm text-teal-600 hover:text-teal-700 font-medium transition-colors"
            ...
            <div class="relative my-6">
                ...
                <div class="relative flex justify-center text-sm">
                    <span class="px-4 bg-white text-slate-500">atau masuk dengan</span>
                </div>
            </div>
        ...
        <div class="mt-8 pt-6 border-t border-slate-100">
            <div class="flex items-center justify-center space-x-2 text-slate-400 text-xs">
                ...
                <span>Dilindungi dengan enkripsi SSL</span>
            </div>
        </div>
    </x-authentication-card>
</x-guest-layout>
