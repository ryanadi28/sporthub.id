<x-app-layout>
    <x-slot name="header">
        ...
        <div class="relative overflow-hidden bg-gradient-to-r from-slate-800 to-slate-900 py-6 px-6 rounded-xl shadow-lg">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h2 class="font-bold text-xl text-white">Booking Saya</h2>
                    <p class="text-teal-300 text-sm">sporthub<span class="text-white">.id</span></p>
                </div>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8 bg-slate-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <!-- New Booking Button -->
            <div class="mb-4">
                <a href="{{ route('customer.booking.index') }}" class="inline-flex items-center space-x-2 bg-teal-500 hover:bg-teal-600 text-white px-5 py-2.5 rounded-lg font-semibold text-sm shadow-md transition-all duration-200">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                        <thead class="bg-slate-800 text-white">
                            <tr>
                                <th class="text-left py-3 px-4 font-semibold uppercase text-xs">Kode Booking</th>
                        ...
                        @forelse($bookings as $b)
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="py-3 px-4 text-center">
                                    @if($b->status === 'pending')
                                        <form method="POST" action="{{ route('customer.booking.cancel', $b) }}" onsubmit="return confirm('Yakin batalkan booking ini?')" class="inline">
                                            @csrf
                                            @method('PUT')
                                    ...
                                    @elseif($b->bukti_transfer)
                                        <a href="{{ asset('storage/' . $b->bukti_transfer) }}" target="_blank" class="inline-flex items-center space-x-1 bg-teal-500 hover:bg-teal-600 text-white px-3 py-1.5 rounded-lg font-medium text-xs transition-colors">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <tr>
                                <td colspan="7" class="py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-slate-600 font-semibold mb-1">Belum Ada Booking</p>
                                        <p class="text-slate-500 text-sm mb-4">Mulai booking lapangan favorit Anda</p>
                                        <a href="{{ route('customer.booking.index') }}" class="inline-flex items-center space-x-2 bg-teal-500 hover:bg-teal-600 text-white px-4 py-2 rounded-lg font-medium text-sm transition-colors">
